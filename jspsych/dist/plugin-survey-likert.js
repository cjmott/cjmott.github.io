var jsPsychSurveyLikert = (function (jspsych) {
  ...
  class SurveyLikertPlugin {
      ...
      trial(display_element, trial) {
          ...
          // logics to only allow submission after all buttons have been clicked
          var response_array = [];
          document.getElementById("jspsych-survey-likert-next").style.visibility = "hidden";
          var radioButtons = document.querySelectorAll('input[type="radio"]');
          radioButtons.forEach(c => {
            c.addEventListener("click", function() {
                response_array.push(c.name[1])
                var unique = response_array.filter((item, i, ar) => ar.indexOf(item) === i);
                if (unique.length==trial.questions.length){
                    document.getElementById("jspsych-survey-likert-next").style.visibility = "visible";
                }
            });
          });
